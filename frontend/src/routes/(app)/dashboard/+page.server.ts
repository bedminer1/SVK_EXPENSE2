...
export const load = async ({ locals }) => {
  const categories = await locals.pb
    ...
    .getFullList<CategoriesReponse>();
  const expensesWithCategory = await locals.pb
    .collection("expenses2")
    .getFullList<ExpensesResponse<{ category: CategoriesReponse }>>({
      expand: "category",
    });
  ...
  const expensesWithCategoryAndImages = expensesWithCategory.map((expense) => {
    ...
    if (icon) {
      expense.expand!.category!.icon = locals.pb.files.getUrl(
  ...
  return {
    expenses: expensesWithCategoryAndImages,
    categories,
  };
};
